...
## Entry 14: Metro cannot resolve the NodeNext import specifiers tsc requires, and a disabled bundling task hid it for three days (2026-09-20)

- Task: rebuild the Fire TV release APK after adding the story library, remote media keys and progress reporting.
- Steps: `npx react-native bundle --platform android --dev false --entry-file index.js ...` from the project path, as `FRICTION_LOG.md` entry 4 prescribes.
- Expected: a bundle, since the same command produced the published one.
- Actual: `error Unable to resolve module ./index.js from packages/karaoke-captions-react/src/native.tsx`. That import is `import { globalWordPosition } from "./index.js"`, and the file on disk is `index.tsx`. TypeScript's NodeNext module resolution requires the `.js` spelling and Node requires it at runtime, so the package is correct; Metro takes the extension literally, finds no `src/index.js`, and fails.
- Severity: high, and it is the *concealment* that earns the rating rather than the resolution gap. Three things lined up. The release variant lists `release` in `debuggableVariants` (our own workaround for entry 4), so the React Native Gradle plugin registers no `createBundleReleaseJsAndAssets` task for it. `index.android.bundle` is therefore generated by hand, and it is not tracked by git. So `assembleRelease` kept succeeding and kept packaging a bundle from two days before the commit that added those extensions. The published APK worked. The app could not be built from its own source, and nothing said so.
- Workaround: a `resolveRequest` in `metro.config.js` that retries a relative `.js` specifier without the extension and falls through if that fails, plus an `npm run bundle` script and a documented two-step release in `tv/README.md` so the step has a name instead of living in a shell history.
- Suggestion, two of them. Metro should resolve relative `.js` specifiers against its own `sourceExts` when the literal file is absent; this is the standard TypeScript NodeNext output shape and every monorepo that publishes typed packages will hit it. Separately, the React Native Gradle plugin should warn at assemble time when a variant has no bundling task and `assets/index.android.bundle` is older than the JavaScript sources, because a stale bundle is invisible: the build is green, the APK installs, and it runs last week's code.
- Why it is in this log: the near miss is the point. Had the emulator run not been part of this audit, a release would have shipped whose JavaScript nobody could regenerate. A build that cannot be reproduced from source is not a build, and ours passed every check we had because none of them rebuilt it.

## Entry 15: a React Native app cannot join npm workspaces, so the primary track's test suite had never run for anybody who cloned the repository (2026-09-20, ours not theirs)

- Task: check, from a clean clone of the public repository, that the documented `npm install && npm test` reaches the number of tests the README claims.
- Steps: `git clone --depth 1 https://github.com/usv240/everyword.git`, then `npm ci`, then `npm test`.
- Expected: the README's figure, covering the caption engine, the MCP server and the Fire TV app.
- Actual: vitest ran 134 and went green. The next command in the chain printed `'jest' is not recognized as an internal or external command`. The Fire TV app sits outside the npm workspaces on purpose, because Metro and Gradle both resolve from the app directory and hoisting breaks them; entries 3, 4 and 5 are all that same hoisting problem. Nothing else installed it, so `tv/node_modules` never existed and the suite on this project's primary track had never run for anyone without an already-working checkout.
- Severity: high, and as with entry 14 it is the concealment that earns the rating. The failure arrives after a long green run, at the very end of the output, phrased like a missing global tool rather than a missing install. The README counted those twelve tests in its headline figure. Somebody who ran the documented command and read the green would have concluded the number was verified.
- Workaround: a `postinstall` at the repository root that installs `tv` as well, plus two assertions that the root `test` script invokes the app's suite and that something in the install chain reaches `tv`, so the published figure and the documented command cannot drift apart again.
