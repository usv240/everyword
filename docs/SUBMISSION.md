...
Fifteen entries in FRICTION_LOG.md, each with task, steps, expected versus actual, severity, workaround, and an actionable suggestion. The ones we would most want read: the Vega SDK has no Windows or WSL support, which combined with Vega OS being Linux-based rather than Android-based leaves a Windows developer with no way to satisfy the "Fire TV/Vega simulator" option at all (entry 12); Polly's best-sounding engine refuses word speech marks, so the product category that most needs both the best voice and word timings cannot have both (entry 7); Builder Tools `init-context` crashes when stdin is not a TTY despite documented non-interactive flags, which blocks any scripted setup (entry 9). There is also a deliberately positive entry for Amazon Transcribe's first-run experience, three Windows Fire TV build failures, and a dataset-integrity lesson that was our own fault and is recorded anyway. Entries 13, 14 and 15 are against us too. Our own deployed MCP server answered a malformed body with an HTTP 500 where JSON-RPC calls for a -32700 parse error, and thirty-five in-process tests passed while the live server was wrong. The Fire TV app could not be rebuilt from its own source for three days while the published APK ran fine, because the release variant has no bundling task and the bundle is untracked. And the documented `npm install && npm test` had never once run the Fire TV app's suite for anybody who cloned the repository, because a React Native app cannot join npm workspaces and nothing installed it. The last two share a lesson worth stating plainly: every check we had was green and the artefact was wrong, and the only thing that found either was leaving the process. One needed the app rebuilt from source. The other needed the repository cloned the way a stranger would clone it.
...
**Fire TV** is the reader's home: a react-native-tvos app targeting Fire OS, shipped as a sideloadable multi-architecture APK carrying the whole five-story library, driven entirely by the remote. The D-pad moves through the library and every control; the remote's media keys play, pause and move five seconds either way; Back returns to the library. Word counts on the cards are computed from the caption documents that drive the highlight, so the number shown is the number counted. Most importantly the television is **a client of this project's own MCP server**: it reports the words the karaoke cursor passed via `record_reading_session`, over the same public endpoint and transport Alexa+ uses and with no shared secret, which is what makes reading progress a measurement rather than something an assistant wrote to itself. Verified end to end on an Android TV virtual device: 113 words read on the television, 113 words returned by `get_reading_progress` from the deployed server. See docs/FIRE_TV_TARGET.md for the target, the test environment, and the honest limits of both.
...
- Fire TV APK and demo footage: https://github.com/usv240/everyword/releases/tag/v0.3.1
