...
The most proven literacy intervention almost nobody in the West has heard of is Same Language Subtitling: subtitles in the same language as the audio, highlighted word by word, karaoke style, on ordinary entertainment. Conceived by Brij Kothari at IIM Ahmedabad and deployed on Indian national television for over two decades, it delivers what researchers call automatic and inescapable reading practice to more than 200 million weak readers, and the broader evidence base runs past 100 studies showing captions improve reading, with at-risk readers gaining most.
...
- **`karaoke-captions-react`**: the renderer, published standalone so any video app can adopt it. Caption document plus a currentTime in, highlighted line out, themed with four CSS variables, with a callback that drives reading-exposure meters. The web reader and the Fire TV app import the same package.
