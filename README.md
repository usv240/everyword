...
- `packages/karaoke-captions-react`: the renderer, an MIT-licensed package published on its own. Its only dependency is `@everyword/captions-core`, also published, so nothing in it needs this repository. Feed it a caption document and a currentTime; theme it with four CSS variables. The web reader and the Fire TV app share it.
