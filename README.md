...
- `apps/mcp`: the **MCP server**, the Alexa+ surface. Model Context Protocol spec 2025-11-25 over Streamable HTTP with five tools: `list_library`, `recommend_story`, `get_reading_progress`, `record_reading_session`, `get_story_text`. Twenty conformance tests. It is a second independent consumer of `captions-core`: word counts and durations are computed from the caption documents rather than trusted from the manifest, so the numbers an agent reports are the numbers the karaoke cursor is driven by.
