...
- `packages/karaoke-captions-react`: the renderer, MIT licensed and packaged to stand alone. Its only dependency is `@everyword/captions-core`, also in this repository and also MIT, so neither needs the application code around them. Both are built and pack cleanly; neither is on npm yet. Feed it a caption document and a currentTime; theme it with four CSS variables. The web reader and the Fire TV app share it.
